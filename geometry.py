...
def _spherical_hdg(ll1,ll2):
    ...
    if np.abs(r_t1) > np.pi:
        r_t1 = (2.*np.pi - np.abs(r_t1))*np.copysign(1.,-r_t1)
...
def _ellipsoidal_hdg(ll1,ll2,r_a=RA,r_e2=RE2):
    ...
    if(r_a*np.abs(r_phi) > 1e-6):    
        ...
        r_sinph0 = np.copysign(1.0,r_sinlam)*np.sqrt(1.0 - r_cosph0**2)
        r_phi0 = np.abs(np.arctan2(r_sinph0,r_cosph0))
...
def llh2xyz(llh,r_a=RA,r_e2=RE2):
    ...
    h = llh[2]
    re = r_a/np.sqrt(1.-r_e2*np.sin(lat)**2)
...
def xyz2llh(xyz,r_a=RA,r_e2=RE2):
    ...
    r_tant = (z + r_q3*r_b*np.sin(r_theta)**3)/(r_p-r_e2*r_a*np.cos(r_theta)**3)
    lat = np.arctan(r_tant)
    r_re = r_a/np.sqrt(1.-r_e2*np.sin(lat)**2)
...
def _reast(r_lat,r_a=RA,r_e2=RE2):
    return r_a/np.sqrt(1. - r_e2*np.sin(r_lat)**2)

def _rnorth(r_lat,r_a=RA,r_e2=RE2):
    return r_a*(1 - r_e2)/(1. - r_e2*np.sin(r_lat)**2)**1.5
...
def sch2xyz(llg,r_a=RA,r_e2=RE2):
    ...
    r_mat = np.array([[r_clt*r_clo,-r_shg*r_slo - r_slt*r_clo*r_chg,r_slo*r_chg - r_slt*r_clo*r_shg],
        [r_clt*r_slo, r_clo*r_shg - r_slt*r_slo*r_chg,  -r_clo*r_chg - r_slt*r_slo*r_shg],
        [r_slt,r_clt*r_chg,r_clt*r_shg]])

    r_matinv = r_mat.T

    r_radcur = _rdir(r_lat,hdg,r_a,r_e2)
    xyz = llh2xyz([llg[0],llg[1],0.]) 
...
def tcnbasis(pos,vel,look_dir='RIGHT',r_a=RA,r_e2=RE2):
    ...
    r_n = -np.array([np.cos(r_lat)*np.cos(r_lon),
