...
def create_db(
    ...
    con = sqlite3.connect(dbfname.strip())
    ...
    startTime=readxmlparam(xmllines,'startTime')
    ...
    sql_mod.edit_param(
            c,swathfile,'slc_file',slcfile,'-','CHAR','raw slc file')
    sql_mod.add_param(c,swathfile,'roi_debug')
    sql_mod.edit_param(
            c,swathfile,'roi_debug',0,'-','INT*4','roi debug parameter')
    numberOfSamplesPerLine=int(readxmlparam(xmllines,'numberOfSamples'))
    numberOfLines=int(readxmlparam(xmllines,'numberOfLines>'))
    ...
    sql_mod.edit_param(c,swathfile,'bytes_per_line',numberOfSamplesPerLine*8,
            '-','INT*4','bytes per line')
    sql_mod.add_param(c,swathfile,'good_bytes_per_line')
    sql_mod.edit_param(c,swathfile,'good_bytes_per_line',
            numberOfSamplesPerLine*8,'-','INT*4','good bytes per line')
    sql_mod.add_param(c,swathfile,'first_line')
    sql_mod.edit_param(c,swathfile,'first_line',0,'-','INT*4',
            'first line (from 0)')
    sql_mod.add_param(c,swathfile,'number_of_patches')
    npatches=1
    sql_mod.edit_param(c,swathfile,'number_of_patches',npatches,'-','INT*4',
            'number of patches')
    sql_mod.add_param(c,swathfile,'iq_pairs_to_skip')
    sql_mod.edit_param(c,swathfile,'iq_pairs_to_skip',0,'-','INT*4',
            'iq pairs to skip')
    ...
    sql_mod.edit_param(c,swathfile,'squint_angle',0,'degrees','REAL*8',
            'squint angle')
    sql_mod.add_param(c,swathfile,'first_range_bin')
    sql_mod.edit_param(c,swathfile,'first_range_bin',1,'-','INT*4',
            'first range bin')
    ...
    sql_mod.edit_param(c,swathfile,'number_of_range_bins',
            numberOfSamplesPerLine,'-','INT*4','number of range bins')
    sql_mod.edit_param(c,swathfile,'range_pixel_delta',0,'pixels','REAL*8',
            'range pixel delta')
    sql_mod.edit_param(c,swathfile,'azimuth_pixel_delta',0,'pixels',
            'real*8','azimuth pixel delta')
    sql_mod.edit_param(c,swathfile,'fd',0,'prfs','real*8',
            'Doppler centroid constant term')

    sql_mod.edit_param(c,swathfile,'fdd',0,'prfs','real*8','Mocomp fdd')
    sql_mod.edit_param(c,swathfile,'fddd',0,'prfs','real*8','Mocomp fddd')
    sql_mod.edit_param(c,swathfile,'caltone_location',0.0,'pct_bw','real*8',
            'Caltone location pct of bw')
    semiMajorAxis=float(readxmlparam(xmllines,'ellipsoidSemiMajorAxis'))
    sql_mod.edit_param(c,swathfile,'re',semiMajorAxis,'m','real*8',
            'Local Earth radius')
    sql_mod.edit_param(c,swathfile,'velocity',7590.,'m/s','real*8',
            'mocomp velocity')
    satelliteHeight=0 #float(readxmlparam(xmllines,'satelliteHeight'))
    sql_mod.edit_param(c,swathfile,'ht',satelliteHeight,'m','real*8',
            'orbit altitude')
    ...
    sql_mod.edit_param(c,swathfile,'azimuth_res',3.,'m','real*8',
            'Azimuth resolution')
    sql_mod.edit_param(c,swathfile,'azimuth_looks',1,'-','INT*4',
            'slc looks = 1')
    #
    # note: Radarsat: for some reason fs needs to be doubled for spotlight mode!
    fs=float(readxmlparam(xmllines,'rangeSamplingRate'))
    sql_mod.edit_param(c,swathfile,'fs',fs,'Hz','real*8',
            'Range sample frequency')
    sql_mod.edit_param(c,swathfile,'srmflag','n','-','CHAR',
            'disable secondary range migration')
    pulseLength=float(readxmlparam(xmllines,'txPulseLength'))
    sql_mod.edit_param(c,swathfile,'pulse_length',pulseLength,'s','real*8',
            'Range chirp length')
    ...
    sql_mod.edit_param(c,swathfile,'radarFrequency',freq,'m','real*8',
            'radar center frequency')

    sql_mod.edit_param(c,swathfile,'weighting',1.0,'-','real*8','Range weighting factor')
    sql_mod.edit_param(c,swathfile,'range_filter',0.0,'-','real*8',
            'range spectrum fractional truncation')
    sql_mod.edit_param(c,swathfile,'resamp_int_r',0.0,'Pixels','real*8',
            'Range resampling intercept')
    sql_mod.edit_param(c,swathfile,'resamp_slope_r',0.0,'Pixels','real*8',
            'Range resampling slope')
    sql_mod.edit_param(c,swathfile,'resamp_int_a',0.0,'Pixels','real*8',
            'Azimuth resampling intercept')
    sql_mod.edit_param(c,swathfile,'resamp_slope_a',0.0,'Pixels','real*8',
            'Azimuth resampling slope')
    sql_mod.edit_param(c,swathfile,'resamp_int_r_delta',0.0,'Pixels','real*8',
            'Range resampling intercept delta')
    sql_mod.edit_param(c,swathfile,'resamp_slope_r_delta',0.0,'Pixels','real*8',
            'Range resampling slope delta')
    sql_mod.edit_param(c,swathfile,'resamp_int_a_delta',0.0,'Pixels','real*8',
            'Azimuth resampling intercept delta')
    sql_mod.edit_param(c,swathfile,'resamp_slope_a_delta',0.0,'Pixels','real*8',
            'Azimuth resampling slope delta')
    ...
    sql_mod.edit_param(c,swathfile,'number_of_az_lines',numberOfLines,'-','int*4',
            'number of azimuth lines')
    sql_mod.add_param(c,swathfile,'patch_size')
    sql_mod.edit_param(c,swathfile,'patch_size',numberOfLines,'-','INT*4',
            'azimuth lines per patch')
    sql_mod.add_param(c,swathfile,'valid_az_samples')
    sql_mod.edit_param(c,swathfile,'valid_az_samples',numberOfLines,'-','INT*4',
            'valid az lines per patch')
    ...
    sql_mod.edit_param(c,swathfile,'lineTimeOrdering',lineTimeOrdering,'-',
            'char','pass direction')
    pixelTimeOrdering=readxmlparam(xmllines,'pixelTimeOrdering')
    sql_mod.add_param(c,swathfile,'pixelTimeOrdering')
    sql_mod.edit_param(c,swathfile,'pixelTimeOrdering',pixelTimeOrdering,'-',
            'char','pass direction')

    # for radarsat save params to account for azimuth-reversed image
    sql_mod.add_param(c,swathfile,'raw_slc_first_line_time')
    sql_mod.edit_param(c,swathfile,'raw_slc_first_line_time',
            timeinseconds(zeroDopplerTimeFirstLine),'seconds','real*8',
            'time of first line in slc product')
    sql_mod.add_param(c,swathfile,'raw_slc_last_line_time')
    sql_mod.edit_param(c,swathfile,'raw_slc_last_line_time',
            timeinseconds(zeroDopplerTimeLastLine),'seconds','real*8',
            'time of last line in slc product')

    # extract ascending/descending 
    passDirection=readxmlparam(xmllines,'passDirection')
    sql_mod.add_param(c,swathfile,'passDirection')
    sql_mod.edit_param(c,swathfile,'passDirection',passDirection,'-','char',
            'pass direction')
    ...
    for i in range(len(start)):
        ...
        lastValidLine.append(
                int(linesPerBurst)-\
                firstsample[halfvalidchars:validchars-1].count('-1')-1)
        firstValidSample.append(
                firstsample.split()[int(len(firstsample.split())/2)])
        lastsample=readxmlparam(burstlines,'lastValidSample')
        lastValidSample.append(
                lastsample.split()[int(len(lastsample.split())/2)])
    ...
    sql_mod.edit_param(c,swathfile,'azimuthBursts',len(azimuthTime),'-','int*4',
            'Bursts in SLC')
    sql_mod.add_param(c,swathfile,'linesPerBurst')
    sql_mod.edit_param(c,swathfile,'linesPerBurst',linesPerBurst,'-','int*4',
            'Bursts in SLC')
    ...
    for i in range(len(azimuthTime)):
        ...
        sql_mod.edit_param(c,swathfile,'azimuthTime'+str(i+1),azimuthTime[i],
                '-','int*4','Burst start time')
        sql_mod.add_param(c,swathfile,'azimuthTimeSeconds'+str(i+1))
        sql_mod.edit_param(c,swathfile,'azimuthTimeSeconds'+str(i+1),
                azimuthTimeSeconds[i],'-','int*4','Burst start time in seconds')
        sql_mod.add_param(c,swathfile,'validLinesPerBurst'+str(i+1))
        sql_mod.edit_param(c,swathfile,'validLinesPerBurst'+str(i+1),
                validLinesPerBurst[i],'-','int*4','valid azimuth lines in burst')
        sql_mod.add_param(c,swathfile,'firstValidLine'+str(i+1))
        sql_mod.edit_param(c,swathfile,'firstValidLine'+str(i+1),
                firstValidLine[i],'-','int*4','first valid azimuth line in burst')
        sql_mod.add_param(c,swathfile,'lastValidLine'+str(i+1))
        sql_mod.edit_param(c,swathfile,'lastValidLine'+str(i+1),lastValidLine[i],
                '-','int*4','last valid azimuth line in burst')
        sql_mod.add_param(c,swathfile,'firstValidSample'+str(i+1))
        sql_mod.edit_param(c,swathfile,'firstValidSample'+str(i+1),
                firstValidSample[i],'-','int*4','first valid range sample in burst')
        sql_mod.add_param(c,swathfile,'lastValidSample'+str(i+1))
        sql_mod.edit_param(c,swathfile,'lastValidSample'+str(i+1),
                lastValidSample[i],'-','int*4','last valid range sample in burst')
    ...
    # this is the first slant range time in imageInformation
    slantRangeTime=float(readxmlparam(xmllines,'slantRangeTime'))
    ...
    sql_mod.edit_param(c,swathfile,'azimuthSteeringRate',azimuthSteeringRate,
            '-','real*8','azimuthSteeringRate')
    sql_mod.edit_param(c,swathfile,'radarFrequency',radarFrequency,'-',
            'real*8','transmit frequency')
    sql_mod.edit_param(c,swathfile,'azimuthTimeInterval',azimuthTimeInterval,
            '-','real*8','azimuth pri')
    sql_mod.edit_param(c,swathfile,'rangeSamplingRate',rangeSamplingRate,'-',
            'real*8','range sample frequency')
    sql_mod.edit_param(c,swathfile,'slantRangeTime',slantRangeTime,'-',
            'real*8','time to near swath')
