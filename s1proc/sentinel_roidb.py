...
def create_db(
    ...
    sql_mod.edit_param(
        c, swathfile, "first_line", 0, "-", "INT*4", "first line (from 0)"
    )
    sql_mod.add_param(c, swathfile, "number_of_patches")
    npatches = 1
    sql_mod.edit_param(
        c, swathfile, "number_of_patches", npatches, "-", "INT*4", "number of patches"
    )
    sql_mod.add_param(c, swathfile, "iq_pairs_to_skip")
    sql_mod.edit_param(
        c, swathfile, "iq_pairs_to_skip", 0, "-", "INT*4", "iq pairs to skip"
    )
    ...
    sql_mod.edit_param(
        c, swathfile, "squint_angle", 0, "degrees", "REAL*8", "squint angle"
    )
    sql_mod.add_param(c, swathfile, "first_range_bin")
    sql_mod.edit_param(
        c, swathfile, "first_range_bin", 1, "-", "INT*4", "first range bin"
    )
    ...
    sql_mod.edit_param(
        c, swathfile, "range_pixel_delta", 0, "pixels", "REAL*8", "range pixel delta"
    )
    sql_mod.edit_param(
        c,
        swathfile,
    ...
    sql_mod.edit_param(
        c, swathfile, "fd", 0, "prfs", "real*8", "Doppler centroid constant term"
    )
    ...
    sql_mod.edit_param(
        c, swathfile, "re", semiMajorAxis, "m", "real*8", "Local Earth radius"
    )
    sql_mod.edit_param(
        c, swathfile, "velocity", 7590.0, "m/s", "real*8", "mocomp velocity"
    )
    satelliteHeight = 0  # float(readxmlparam(xmllines,'satelliteHeight'))
    sql_mod.edit_param(
        c, swathfile, "ht", satelliteHeight, "m", "real*8", "orbit altitude"
    )
    ...
    sql_mod.edit_param(
        c, swathfile, "azimuth_res", 3.0, "m", "real*8", "Azimuth resolution"
    )
    ...
    sql_mod.edit_param(
        c, swathfile, "srmflag", "n", "-", "CHAR", "disable secondary range migration"
    )
    pulseLength = float(readxmlparam(xmllines, "txPulseLength"))
    sql_mod.edit_param(
        c, swathfile, "pulse_length", pulseLength, "s", "real*8", "Range chirp length"
    )
    ...
    sql_mod.edit_param(
        c, swathfile, "radarFrequency", freq, "m", "real*8", "radar center frequency"
    )

    sql_mod.edit_param(
        c, swathfile, "weighting", 1.0, "-", "real*8", "Range weighting factor"
    )
    sql_mod.edit_param(
        c,
        swathfile,
    ...
    sql_mod.edit_param(
        c, swathfile, "passDirection", passDirection, "-", "char", "pass direction"
    )
    ...
    for i in range(len(start)):
        ...
        endTimeSeconds.append(
            timeinseconds(timestring) + (int(linesPerBurst) - 1) / prf
        )
        # valid data region
        ...
        lastValidLine.append(
            int(linesPerBurst)
            - firstsample[halfvalidchars : validchars - 1].count("-1")
            - 1
        )
    ...
    sql_mod.edit_param(
        c, swathfile, "azimuthBursts", len(azimuthTime), "-", "int*4", "Bursts in SLC"
    )
    sql_mod.add_param(c, swathfile, "linesPerBurst")
    sql_mod.edit_param(
        c, swathfile, "linesPerBurst", linesPerBurst, "-", "int*4", "Bursts in SLC"
    )
    sql_mod.add_param(c, swathfile, "samplesPerBurst")
    sql_mod.edit_param(
        c, swathfile, "samplesPerBurst", samplesPerBurst, "-", "int*4", "Bursts in SLC"
    )
    ...
    for i in range(len(azimuthTime)):
        ...
        sql_mod.edit_param(
            c,
            swathfile,
            "azimuthTimeSeconds" + str(i + 1),
            azimuthTimeSeconds[i],
            "-",
            "int*4",
        ...
        sql_mod.edit_param(
            c,
            swathfile,
            "validLinesPerBurst" + str(i + 1),
            validLinesPerBurst[i],
            "-",
            "int*4",
            "valid azimuth lines in burst",
        )
        sql_mod.add_param(c, swathfile, "firstValidLine" + str(i + 1))
        sql_mod.edit_param(
            c,
            swathfile,
            "firstValidLine" + str(i + 1),
            firstValidLine[i],
            "-",
            "int*4",
            "first valid azimuth line in burst",
        )
        sql_mod.add_param(c, swathfile, "lastValidLine" + str(i + 1))
        sql_mod.edit_param(
            c,
            swathfile,
            "lastValidLine" + str(i + 1),
            lastValidLine[i],
            "-",
            "int*4",
            "last valid azimuth line in burst",
        )
        sql_mod.add_param(c, swathfile, "firstValidSample" + str(i + 1))
        sql_mod.edit_param(
            c,
            swathfile,
            "firstValidSample" + str(i + 1),
            firstValidSample[i],
            "-",
            "int*4",
            "first valid range sample in burst",
        )
        sql_mod.add_param(c, swathfile, "lastValidSample" + str(i + 1))
        sql_mod.edit_param(
            c,
            swathfile,
            "lastValidSample" + str(i + 1),
            lastValidSample[i],
            "-",
            "int*4",
            "last valid range sample in burst",
        )
    ...
    # ks = 2 * 7500 / 299792458.0 * radarFrequency * azimuthSteeringRate * np.pi / 180.0
