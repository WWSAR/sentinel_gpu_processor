...
class GeoCoordinates:
    def __init__(self, rscfile=None, rscparams=None, metadatafile=None):
        if rscfile is not None:
            ...
            self.dlon = 1.0 / 7200
            self.dlat = -1.0 / 7200
            self.nlon = np.round((self.lonmax - self.lonmin) / self.dlon + 1).astype(
                int
            )
            self.nlat = np.round((self.latmin - self.latmax) / self.dlat + 1).astype(
                int
            )
        else:
        ...
        self.lonspacing = (
            RA * np.cos((self.latmin + self.latmax) * np.pi / 180 / 2) * np.pi / 180
        )
    ...
    def grid(self):
        return np.mgrid[
            self.latmax : self.latmin : (self.nlat * 1j),
            self.lonmin : self.lonmax : (self.nlon * 1j),
        ]

    def tokml(
        self, kmlfilename="roi", img=None, name="roi", description="Region of Interest"
    ):
...
<LatLonBox>
    ...
    def take_look(self, latlook, lonlook):
        ...
        rscparams["latmax"] = self.latmax + self.dlat * (latlook - 1.0) / 2
        rscparams["lonmin"] = self.lonmin + self.dlon * (lonlook - 1.0) / 2
