...
// io functions
void read_polynomials(const std::string &fname, int &n, double **t, double **t0,
                      double **p0, double **p1, double **p2);

template <typename T>
void read_binary(const std::string &imgfile, const size_t width,
                 const size_t row_start, const size_t row_end,
                 const size_t col_start, const size_t col_end, T *img) {
  ...
  for (size_t r = 0; r < sub_height; r++) {
    ...
    size_t file_offset = ((row_start + r) * width + col_start) * sizeof(T);
...
template <typename T>
void read_binary(const std::string &imgfile, const std::size_t toskip,
                 const std::size_t n, T *img) {
  std::ifstream fin(imgfile, std::ios::binary);
...
template <typename T>
void read_binary(const std::string &imgfile, const std::size_t n, T *img) {
  read_binary<T>(imgfile, 0, n, img);
}

template <typename T>
void save_binary(T *img, bool append, const std::size_t n,
...
template <typename T>
void save_binary(const T *img, const std::size_t toskip, const std::size_t n,
                 const std::string &imgfile) {
  std::fstream fout;
...
template <typename T>
void save_binary(T *img, const std::size_t n, const std::string &imgfile) {
  save_binary<T>(img, false, n, imgfile);
}

template <typename T>
void save_binary(const T *img, const std::size_t n, const std::int32_t *header,
                 const std::size_t nheader, const std::string &imgfile) {
  std::ofstream fout;
...
*/
template <typename T>
void read_and_resample(const std::string &filename, T *dst, const int left_src,
                       const int top_src, const int right_src,
                       const int bottom_src, const int offset_rows,
                       const int left_dst, const int top_dst,
                       const int right_dst, const int bottom_dst,
  ...
  size_t offset = header_bytes + (size_t)src_row0 * src_w * sizeof(T);
...
template <typename T>
void read_and_resample(const std::string &filename, T *dst, const int left_dst,
                       const int top_dst, const int right_dst,
                       const int bottom_dst, const int row_begin,
                       const int row_end) {
  std::ifstream fin(filename, std::ios::binary);
  if (!fin)
    throw std::runtime_error("Cannot open file in read_and_resample");
  ...
  read_and_resample<T>(filename, dst, left_src, top_src, right_src, bottom_src,
                       0, left_dst, top_dst, right_dst, bottom_dst, row_begin,
                       row_end);
}

void write_compressed_strips(int32_t *header, const float2 *img, const int nrow,
                             const int ncol, const int top,
                             const std::string &outfile);

// classes
template <typename T> class Strip {
public:
  int nrow0;
  int ncol0;
  int left;
  ...
  Strip(const int nrow0_, const int ncol0_, const int left_, const int top_,
        const int right_, const int bottom_, const int start_line_,
        const std::string &fname_, T *data_);
  ...
  Strip(const Strip &other)
      : nrow0(other.nrow0), ncol0(other.ncol0), left(other.left),
        top(other.top), right(other.right), bottom(other.bottom),
        start_line(other.start_line), nrow(other.nrow), ncol(other.ncol),
        fname(other.fname), data(other.data) {}
  ...
  Strip(Strip &&other) noexcept
      : nrow0(other.nrow0), ncol0(other.ncol0), left(other.left),
        top(other.top), right(other.right), bottom(other.bottom),
        start_line(other.start_line), nrow(other.nrow), ncol(other.ncol),
        fname(std::move(other.fname)), data(other.data) {
    other.data = nullptr;
  }
...
template <typename T>
Strip<T>::Strip(const int nrow0_, const int ncol0_, const int left_,
                const int top_, const int right_, const int bottom_,
                const int start_line_, const std::string &fname_, T *data_)
    : nrow0(nrow0_), ncol0(ncol0_), left(left_), top(top_), right(right_),
      start_line(start_line_), bottom(bottom_), fname(fname_), data(data_) {
  nrow = bottom - top;
  ncol = right - left;
}
...
template <typename T>
void Strip<T>::load_data(const int left, const int top, const int right,
                         const int bottom) {
  if (this->data) {
    free(this->data);
    this->data = nullptr;
  ...
  read_and_resample<T>(this->fname, this->data, this->left, this->top,
                       this->right, this->bottom, this->start_line, left, top,
                       right, bottom, 0, bottom - top);
  return;
}

template <typename T> void Strip<T>::save_data() {
  if (!this->data) {
  ...
  save_binary<T>(this->data, this->nrow * this->ncol, header, NHEADER,
                 this->fname);
}

template <typename T> class Subswath {
public:
  int nrow0;
  int ncol0;
  int left;
...
Subswath<T>::Subswath(const std::string &fname_) : fname(fname_) {
  ...
  for (int i = 0; i < nstrip; i++) {
    ...
    Strip<T> s(nrow0, ncol0, left, top, right, bottom, start_line, fname,
               nullptr);
    data.push_back(s);
...
__global__ void point_sqrt(float *a, float *b, const std::size_t n);

__global__ void point_angle(Complex *z, float *phase, const std::size_t n);

__global__ void cpx_col_look(Complex *a, Complex *b, const int collook,
                             const std::size_t ncol, const std::size_t n);

__global__ void cpx_row_look(Complex *a, Complex *b, const int rowlook,
                             const std::size_t ncol, const std::size_t n);

__global__ void col_look(float *a, float *b, const int collook,
                         const std::size_t ncol, const std::size_t n);
