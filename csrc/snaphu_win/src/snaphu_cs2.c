...
#define TIME_FOR_UPDATE (n_rel > n * UPDT_FREQ + n_src * UPDT_FREQ_S)
...
long n_push = 0, n_relabel = 0, n_discharge = 0, n_refine = 0, n_update = 0,
     n_scan = 0, n_prscan = 0, n_prscan1 = 0, n_prscan2 = 0, n_bad_pricein = 0,
     n_bad_relabel = 0, n_prefine = 0;
...

{
  ...
  long i_rank, j_rank, /* ranks of nodes */
      j_new_rank;
    ...
    if (OPEN(ra)) {
      ...
      if (j_rank > i_rank) {
        if ((rc = REDUCED_COST(j, i, ra)) < 0)
          ...
          dr = rc / epsilon;
          j_new_rank = (dr < dlinf) ? i_rank + (long)dr + 1 : linf;
        }
...

{
  ...
  for (a = i->current + 1, a_stop = (i + 1)->suspended; a != a_stop; a++) {
    if (OPEN(a) && ((dp = ((a->head)->price) - dn * (a->cost)) > p_max)) {
      if (i_price < dp) {
        i->current = a;
        return (1);
      }
...

{
  ...
  while (1) {
    ...
    if (j_exc >= 0) {
      b = j->current;
      if (ADMISSIBLE(j, b->head, b) || relabel(j)) { /* exit from j exists */
...

{
  ...
  while (1) { /* while negative cycle is found or eps-optimal solution is
                 constructed */
          ...
          if (OPEN(a)) {
            ...
            if (j_rank < i_rank) {
              ...
              if (rc < 0)
                ...
                dr = rc / epsilon;
                j_new_rank = (dr < dlinf) ? i_rank - ((long)dr + 1) : 0;
              }
              if (j_rank < j_new_rank) {
                if (cc == 1) {
                  j->rank = j_new_rank;
...
void SolveCS2(signed char **residue, short **mstcosts, long nrow, long ncol,
  ...
  f_sc = cs2scalefactor;
  cs2mcfparse(residue, rowcost, colcost, nNrow, nNcol, &n, &m, &ndp, &arp,
              &nmin, &c_max, &cap);
    ...
    for (a = i->suspended; a != (i + 1)->suspended; a++) {
      ...
      if (cap[N_ARC(a)] > 0 && (cap[N_ARC(a)] - (a->r_cap))) {
        ...
        if ((from == ground) || (to == ground)) {
          ...
          colflow[(num - 1) % (nNrow + 1)][(int)(num - 1) / (nNrow + 1)] -=
              flow;
        } else if (from == (to - 1)) {
