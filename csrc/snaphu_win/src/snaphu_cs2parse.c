...
#define EN22 20
  ...
  narcs = 2 * ((nNrow + 1) * nNcol +
               nNrow * (nNcol + 1)); /* 2x for two directional arcs */
  cumsupply = 0;
  nresidues = 0;
  ...
  for (col = 0; col < nNcol; col++) {
    for (row = 0; row < nNrow; row++) {
      if (residue[row][col]) {
        ...
        if (excess > 0)
          total_p += (double)excess;
        if (excess < 0)
          total_n -= (double)excess;
        nresidues++;
  ...
  if (cumsupply < 0)
    total_p -= (double)cumsupply;
  if (cumsupply > 0)
    total_n += (double)cumsupply;
  ...
  for (arcctr = 1; arcctr <= 2 * nNrow * nNcol + nNrow + nNcol; arcctr++) {
    if (arcctr <= nNrow * (nNcol + 1)) {
      ...
      cost = colcost[((nodectr - 1) % (nNrow + 1))]
                    [(int)((nodectr - 1) / (nNrow + 1))];
    }
