...
static void **BuildStatCostsTopo(float **wrappedphase, float **mag,
                                 float **unwrappedest, float **pwr,
                                 float **corr, short **rowweight,
                                 short **colweight, long nrow, long ncol,
                                 tileparamT *tileparams, outfileT *outfiles,
                                 paramT *params);
static void **BuildStatCostsDefo(float **wrappedphase, float **mag,
...
static void **BuildStatCostsSmooth(float **wrappedphase, float **mag,
                                   float **unwrappedest, float **corr,
                                   short **rowweight, short **colweight,
                                   long nrow, long ncol, tileparamT *tileparams,
                                   outfileT *outfiles, paramT *params);
static void MaskCost(costT *costptr);
static void MaskSmoothCost(smoothcostT *smoothcostptr);
static int MaskPrespecifiedArcCosts(void **costsptr, short **weights, long nrow,
                                    long ncol, paramT *params);
static int GetIntensityAndCorrelation(float **mag, float **wrappedphase,
                                      float ***pwrptr, float ***corrptr,
                                      infileT *infiles, long linelen,
                                      long nlines, long nrow, long ncol,
                                      outfileT *outfiles, paramT *params,
                                      tileparamT *tileparams);
static int RemoveMean(float **ei, long nrow, long ncol, long krowei,
                      long kcolei);
static float *BuildDZRCritLookupTable(double *nominc0ptr, double *dnomincptr,
                                      long *tablesizeptr,
                                      tileparamT *tileparams, paramT *params);
static double SolveDZRCrit(double sinnomincangle, double cosnomincangle,
                           paramT *params, double threshold);
static int SolveEIModelParams(double *slope1ptr, double *slope2ptr,
                              double *const1ptr, double *const2ptr,
                              double dzrcrit, double dzr0,
                              double sinnomincangle, double cosnomincangle,
                              paramT *params);
static double EIofDZR(double dzr, double sinnomincangle, double cosnomincangle,
                      paramT *params);
static float **BuildDZRhoMaxLookupTable(double nominc0, double dnominc,
                                        long nominctablesize, double rhomin,
                                        double drho, long nrho, paramT *params);
static double CalcDZRhoMax(double rho, double nominc, paramT *params,
                           double threshold);
static int CalcInitMaxFlow(paramT *params, void **costs, long nrow, long ncol);
 ...
 */
int BuildCostArrays(void ***costsptr, short ***mstcostsptr, float **mag,
                    float **wrappedphase, float **unwrappedest, long linelen,
                    long nlines, long nrow, long ncol, paramT *params,
  ...
  void (*CalcStatCost)(void **, long, long, long, long, long, paramT *, long *,
                       long *);
  ...
  if (strlen(infiles->costinfile)) {
    ...
    fprintf(sp1, "Reading cost information from file %s\n",
            infiles->costinfile);
    costs = NULL;
    Read2DRowColFile((void ***)&costs, infiles->costinfile, linelen, nlines,
                     tileparams, costtypesize);
    (*costsptr) = costs;
    ...
    GetIntensityAndCorrelation(mag, wrappedphase, &pwr, &corr, infiles, linelen,
                               nlines, nrow, ncol, outfiles, params,
                               tileparams);
  ...
  if (strlen(infiles->costinfile) || params->costmode != NOSTATCOSTS) {
    if (strlen(outfiles->costoutfile)) {
      Write2DRowColArray((void **)costs, outfiles->costoutfile, nrow, ncol,
                         costtypesize);
    } else {
      if (strlen(outfiles->rowcostfile)) {
        Write2DArray((void **)rowcost, outfiles->rowcostfile, nrow - 1, ncol,
                     costtypesize);
      }
      if (strlen(outfiles->colcostfile)) {
        Write2DArray((void **)colcost, outfiles->colcostfile, nrow, ncol - 1,
                     costtypesize);
      }
    }
  }
  ...
  if (params->p >= 0) {
    if (params->bidirlpn) {
      bidircosts = (bidircostT **)Get2DRowColMem(
          nrow, ncol, sizeof(bidircostT *), sizeof(bidircostT));
      (*costsptr) = (void **)bidircosts;
    } else {
      scalarcosts =
          (short **)Get2DRowColMem(nrow, ncol, sizeof(short *), sizeof(short));
  ...
  if (params->costmode == NOSTATCOSTS) {
    ...
    for (row = 0; row < 2 * nrow - 1; row++) {
      ...
      for (col = 0; col < maxcol; col++) {
        ...
        CalcStatCost((void **)costs, 0, row, col, 1, nrow, params, &poscost,
                     &negcost);
    ...
    if (strlen(outfiles->mstrowcostfile)) {
      Write2DArray((void **)rowweight, outfiles->mstrowcostfile, nrow - 1, ncol,
                   sizeof(short));
    }
    if (strlen(outfiles->mstcolcostfile)) {
      Write2DArray((void **)colweight, outfiles->mstcolcostfile, nrow, ncol - 1,
                   sizeof(short));
    }
    if (strlen(outfiles->mstcostsfile)) {
      Write2DRowColArray((void **)rowweight, outfiles->mstcostsfile, nrow, ncol,
                         sizeof(short));
    }
...
static void **BuildStatCostsTopo(float **wrappedphase, float **mag,
  ...
  costscale =
      params->costscale * fabs(params->costscaleambight / midrangeambight);
  ...
  CalcWrappedRangeDiffs(dpsi, avgdpsi, wrappedphase, kperpdpsi, kpardpsi, nrow,
                        ncol);
  ...
  for (col = 0; col < ncol - 1; col++) {
    ...
    cosnomincangle =
        (a * a - slantrange * slantrange - re * re) / (2 * slantrange * re);
    ...
    for (row = 0; row < nrow; row++) {
      ...
      if (colweight[row][col] == 0) {
        ...
        if (rho > 0) {
          dzrhomax = LinInterp2D(dzrhomaxtable, nomincind,
                                 (rho - rhomin) / drho, nominctablesize, nrho);
          if (dzrhomax < dzlay) {
            dzlay = dzrhomax;
          }
        }
        ...
        if (dzlay) {
          if (rho > 0) {
            colcost[row][col].offset =
                nshortcycle *
                (dpsi[row][col] - 0.5 * (avgdpsi[row][col] + dphilaypeak));
          } else {
            colcost[row][col].offset =
                nshortcycle * (dpsi[row][col] - 0.25 * avgdpsi[row][col] -
                               0.75 * dphilaypeak);
          }
          colcost[row][col].sigsq = (sigsqrho + sigsqei + sigsqlay) *
                                    ztoshortsq /
          ...
          if (labs(colcost[row][col].dzmax) >
              floor(
                  sqrt(colcost[row][col].laycost * colcost[row][col].sigsq))) {
            nolayover = FALSE;
          }
        }
        if (nolayover) {
        ...
        if (unwrappedest != NULL) {
          colcost[row][col].offset +=
              (nshortcycle / TWOPI *
               (unwrappedest[row][col + 1] - unwrappedest[row][col]));
        }
      }
    }
  ...
  CalcWrappedAzDiffs(dpsi, avgdpsi, wrappedphase, kperpdpsi, kpardpsi, nrow,
                     ncol);
  ...
  for (col = 0; col < ncol; col++) {
    ...
    for (row = 0; row < nrow - 1; row++) {
      ...
      if (rowweight[row][col] == 0) {
        ...
        /* set cost parameters in terms of flow, represented as shorts */
        if (rho > 0) {
          rowcost[row][col].offset =
              nshortcycle * (dpsi[row][col] - avgdpsi[row][col]);
        } else {
          rowcost[row][col].offset =
              nshortcycle * (dpsi[row][col] - 0.5 * avgdpsi[row][col]);
        }
        nolayover = TRUE;
        if (dzlay) {
          rowcost[row][col].sigsq = (sigsqrho + sigsqei + sigsqlay) *
                                    ztoshortsq /
          ...
          if (labs(rowcost[row][col].dzmax) >
              floor(
                  sqrt(rowcost[row][col].laycost * rowcost[row][col].sigsq))) {
            nolayover = FALSE;
          }
        }
        if (nolayover) {
        ...
        if (unwrappedest != NULL) {
          rowcost[row][col].offset +=
              (nshortcycle / TWOPI *
               (unwrappedest[row + 1][col] - unwrappedest[row][col]));
        }
      }
    }
  ...
  for (col = 0; col < ncol - 1; col++) {
    for (row = 0; row < nrow; row++) {
      ...
      if (colweight[row][col] == 0) {
        ...
        sigsqrho =
            (sigsqrhoconst * pow(1 - rho, rhopow) + sigsqcorr) * nshortcyclesq;
...
static int GetIntensityAndCorrelation(float **mag, float **wrappedphase,
  ...
  if (strlen(infiles->ampfile)) {
    ReadIntensity(&pwr, &pwr1, &pwr2, infiles, linelen, nlines, params,
                  tileparams);
  } else {
    if (params->costmode == TOPO) {
  ...
  if (strlen(infiles->corrfile)) {
    ...
    krowcorr =
        1 + 2 * floor(params->ncorrlooksaz / (double)params->nlooksaz / 2);
    kcolcorr = 1 + 2 * floor(params->ncorrlooksrange /
                             (double)params->nlooksrange / 2);

    /* calculate equivalent number of independent looks */
    params->ncorrlooks = (kcolcorr * (params->dr / params->rangeres)) *
                         (krowcorr * (params->da / params->azres)) *
                         params->nlooksother;
    ...
    padreal =
        MirrorPad(realcomp, nrow, ncol, (krowcorr - 1) / 2, (kcolcorr - 1) / 2);
    padimag =
        MirrorPad(imagcomp, nrow, ncol, (krowcorr - 1) / 2, (kcolcorr - 1) / 2);
    ...
    padpwr1 =
        MirrorPad(pwr1, nrow, ncol, (krowcorr - 1) / 2, (kcolcorr - 1) / 2);
    avgpwr1 = pwr1;
    BoxCarAvg(avgpwr1, padpwr1, nrow, ncol, krowcorr, kcolcorr);
    padpwr2 =
        MirrorPad(pwr2, nrow, ncol, (krowcorr - 1) / 2, (kcolcorr - 1) / 2);
    avgpwr2 = pwr2;
  ...
  if (strlen(outfiles->rawcorrdumpfile)) {
    Write2DArray((void **)corr, outfiles->rawcorrdumpfile, nrow, ncol,
                 sizeof(float));
  }
  ...
  if (strlen(outfiles->corrdumpfile)) {
    Write2DArray((void **)corr, outfiles->corrdumpfile, nrow, ncol,
                 sizeof(float));
  }
...
static float *BuildDZRCritLookupTable(double *nominc0ptr, double *dnomincptr,
  ...
  nominc0 =
      acos((a * a - slantrange * slantrange - re * re) / (2 * slantrange * re));
...
static double SolveDZRCrit(double sinnomincangle, double cosnomincangle,
  ...
  while (TRUE) {
    ...
    if (fabs(residual =
                 costhetai - (dzr * sinnomincangle + dx * cosnomincangle) /
                                 sqrt(dzr * dzr + dx * dx)) <
        threshold * costhetai) {
      return (dzr);
    }
 ...
 * relationship.
 */
int SolveEIModelParams(double *slope1ptr, double *slope2ptr, double *const1ptr,
                       double *const2ptr, double dzrcrit, double dzr0,
                       double sinnomincangle, double cosnomincangle,
                       paramT *params) {
...
static float **BuildDZRhoMaxLookupTable(double nominc0, double dnominc,
  ...
  dzrhomaxtable =
      (float **)Get2DMem(nominctablesize, nrho, sizeof(float *), sizeof(float));
  nominc = nominc0;
  for (knominc = 0; knominc < nominctablesize; knominc++) {
    rho = rhomin;
    for (krho = 0; krho < nrho; krho++) {
      dzrhomaxtable[knominc][krho] =
          (float)CalcDZRhoMax(rho, nominc, params, params->threshold);
      rho += drho;
    }
    nominc += dnominc;
  }
...
static double CalcDZRhoMax(double rho, double nominc, paramT *params,
  ...
  rhosfactor =
      2.0 * fabs(bperp) * (params->rangeres) / ((params->lambda) * slantrange);
 ...
 */
void CalcCostTopo(void **costs, long flow, long arcrow, long arccol, long nflow,
                  long nrow, paramT *params, long *poscostptr,
                  long *negcostptr) {
 ...
 */
void CalcCostDefo(void **costs, long flow, long arcrow, long arccol, long nflow,
                  long nrow, paramT *params, long *poscostptr,
                  long *negcostptr) {
...
void CalcCostSmooth(void **costs, long flow, long arcrow, long arccol,
                    long nflow, long nrow, paramT *params, long *poscostptr,
                    long *negcostptr) {
 ...
 */
void CalcCostL0(void **costs, long flow, long arcrow, long arccol, long nflow,
                long nrow, paramT *params, long *poscostptr, long *negcostptr) {
 ...
 */
void CalcCostL1(void **costs, long flow, long arcrow, long arccol, long nflow,
                long nrow, paramT *params, long *poscostptr, long *negcostptr) {

  /* L1-norm */
  *poscostptr =
      ((short **)costs)[arcrow][arccol] * (labs(flow + nflow) - labs(flow));
  *negcostptr =
      ((short **)costs)[arcrow][arccol] * (labs(flow - nflow) - labs(flow));
 ...
 */
void CalcCostL2(void **costs, long flow, long arcrow, long arccol, long nflow,
                long nrow, paramT *params, long *poscostptr, long *negcostptr) {
 ...
 */
void CalcCostLP(void **costs, long flow, long arcrow, long arccol, long nflow,
                long nrow, paramT *params, long *poscostptr, long *negcostptr) {
...
void CalcCostL1BiDir(void **costs, long flow, long arcrow, long arccol,
  ...
  if (newflow > 0) {
    *poscostptr =
        (((bidircostT **)costs)[arcrow][arccol].posweight * newflow - cost0);
  } else {
    *poscostptr =
        (-((bidircostT **)costs)[arcrow][arccol].negweight * newflow - cost0);
  }
  newflow = flow - nflow;
  if (newflow > 0) {
    *negcostptr =
...
void CalcCostL2BiDir(void **costs, long flow, long arcrow, long arccol,
  ...
  if (newflow > 0) {
    *poscostptr =
        (((bidircostT **)costs)[arcrow][arccol].posweight * newflow * newflow -
         cost0);
  } else {
    *poscostptr =
        (((bidircostT **)costs)[arcrow][arccol].negweight * newflow * newflow -
         cost0);
  }
  newflow = flow - nflow;
  if (newflow > 0) {
...
long EvalCostTopo(void **costs, short **flows, long arcrow, long arccol,
  ...
  if (idz1 > dzmax) {
    idz1 -= dzmax;
    cost1 = (idz1 * idz1) / ((params->layfalloffconst) * (cost->sigsq)) +
            cost->laycost;
  } else {
    cost1 = (idz1 * idz1) / cost->sigsq;
