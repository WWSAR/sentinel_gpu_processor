...
#ifdef _WIN32
#ifndef SIGPIPE
#define SIGPIPE 13 /* Classic POSIX value, unused by Windows kernels */
#endif
#ifndef SIGALRM
#define SIGALRM 14 /* Classic POSIX value, unused by Windows kernels */
#endif
#endif
#include <stdio.h>
#include <stdlib.h>
...
static int ParseConfigLine(char *buf, char *conffile, long nlines,
                           infileT *infiles, outfileT *outfiles,
                           long *linelenptr, paramT *params);
static int LogStringParam(FILE *fp, char *key, char *value);
static int LogBoolParam(FILE *fp, char *key, signed char boolvalue);
static int LogFileFormat(FILE *fp, char *key, signed char fileformat);
static int WriteAltLineFile(float **mag, float **phase, char *outfile,
                            long nrow, long ncol);
static int WriteAltSampFile(float **arr1, float **arr2, char *outfile,
                            long nrow, long ncol);
        ...
        for (j = 1; j < strlen(argv[i]); j++) {
          if (argv[i][j] == 'h') {
            ...
            if (++i < argc && j == strlen(argv[i - 1]) - 1) {
              ...
              ParseConfigLine(argv[i], "command line", 0, infiles, outfiles,
                              linelenptr, params);
              break;
            } else {
              noarg_exit = TRUE;
            ...
            if (++i < argc && j == strlen(argv[i - 1]) - 1) {
              if (StringToDouble(argv[i], &(params->bperp)) ||
                  !(params->bperp)) {
                fflush(NULL);
 ...
 */
int CheckParams(infileT *infiles, outfileT *outfiles, long linelen, long nlines,
                paramT *params) {
  ...
  if ((fp = fopen(outfiles->outfile, "a")) == NULL) {
    ...
    if (!strcmp(outfiles->outfile, infiles->infile) && !params->eval &&
  ...
  */
  ...
  if (params->costmode == TOPO &&
      (params->maxflow * params->nshortcycle) > POSSHORTRANGE) {
    fflush(NULL);
    fprintf(sp0, "maxflow exceeds range of short int for given nshortcycle\n");
    exit(ABNORMAL_EXIT);
  }
  if (params->costmode == DEFO &&
      ceil(params->defomax * params->nshortcycle) > POSSHORTRANGE) {
    fflush(NULL);
  ...
  if (params->maxnflowcycles == USEMAXCYCLEFRACTION) {
    params->maxnflowcycles =
        LRound(params->maxcyclefraction * nlines / (double)params->ntilerow *
  ...
  /* masking parameters */
  if (strlen(infiles->bytemaskfile) || params->edgemasktop ||
      params->edgemaskbot || params->edgemaskleft || params->edgemaskright) {
    if (params->initonly) {
      fflush(NULL);
...
int ReadConfigFile(char *conffile, infileT *infiles, outfileT *outfiles,
  ...
  while (TRUE) {
    ...
    /* parse config line */
    parsestatus = ParseConfigLine(buf, conffile, nlines, infiles, outfiles,
                                  linelenptr, params);
    if (parsestatus > 0) {
      nparams++;
    }
...
static int ParseConfigLine(char *buf, char *conffile, long nlines,
  ...
  if (nfields == 2 && isalnum(str1[0])) {
    ...
    if (!strcmp(str1, "INFILE")) {
      ...
      } else if (!strcmp(str2, "SINGLEANTENNATRANSMIT") ||
                 !strcmp(str2, "SAT") || !strcmp(str2, "SINGLEANTTRANSMIT")) {
...
int WriteConfigLogFile(int argc, char *argv[], infileT *infiles,
  ...
  if (strlen(outfiles->logfile)) {
    ...
    /* open the log file */
    if ((fp = fopen(outfiles->logfile, "wb")) == NULL) {
      fflush(NULL);
 ...
 */
int Write2DRowColArray(void **array, char *filename, long nrow, long ncol,
                       size_t size) {
...
int ReadInputFile(infileT *infiles, float ***magptr, float ***wrappedphaseptr,
                  short ***flowsptr, long linelen, long nlines, paramT *params,
  ...
  if (!params->unwrapped) {
    ...
    if (infiles->infileformat == COMPLEX_DATA) {
      ReadComplexFile(&mag, &wrappedphase, infiles->infile, linelen, nlines,
                      tileparams);
    } else if (infiles->infileformat == ALT_LINE_DATA) {
      ReadAltLineFile(&mag, &wrappedphase, infiles->infile, linelen, nlines,
                      tileparams);
    } else if (infiles->infileformat == ALT_SAMPLE_DATA) {
      ReadAltSampFile(&mag, &wrappedphase, infiles->infile, linelen, nlines,
                      tileparams);
    ...
    if (infiles->unwrappedinfileformat == ALT_LINE_DATA) {
      ReadAltLineFile(&mag, &unwrappedphase, infiles->infile, linelen, nlines,
                      tileparams);
    } else if (infiles->unwrappedinfileformat == ALT_SAMPLE_DATA) {
      ReadAltSampFile(&mag, &unwrappedphase, infiles->infile, linelen, nlines,
                      tileparams);
  ...
  if (tileparams->nrow != nlines || tileparams->ncol != linelen) {
    fprintf(sp2, "Read %ldx%ld array of pixels starting at row,col %ld,%ld\n",
            tileparams->nrow, tileparams->ncol, tileparams->firstrow,
            tileparams->firstcol);
  }
  ...
  for (row = 0; row < nrow; row++) {
    for (col = 0; col < ncol; col++) {
      ...
      if ((bytemask != NULL && bytemask[row][col] == 0) ||
          fullrow < params->edgemasktop || fullcol < params->edgemaskleft ||
...
int ReadUnwrappedEstimateFile(float ***unwrappedestptr, infileT *infiles,
  ...
  if (infiles->estfileformat == ALT_LINE_DATA) {
    ReadAltLineFilePhase(unwrappedestptr, infiles->estfile, linelen, nlines,
                         tileparams);
    ...
    ReadAltSampFile(&dummy, unwrappedestptr, infiles->estfile, linelen, nlines,
                    tileparams);
  } else {
    fflush(NULL);
 ...
 */
int ReadWeightsFile(short ***weightsptr, char *weightfile, long linelen,
                    long nlines, tileparamT *tileparams) {
...
int ReadIntensity(float ***pwrptr, float ***pwr1ptr, float ***pwr2ptr,
                  infileT *infiles, long linelen, long nlines, paramT *params,
                  tileparamT *tileparams) {
  ...
  if (strlen(infiles->ampfile2)) {
    ...
    if (infiles->ampfileformat == FLOAT_DATA) {
      Read2DArray((void ***)&pwr1, infiles->ampfile, linelen, nlines,
                  tileparams, sizeof(float *), sizeof(float));
      Read2DArray((void ***)&pwr2, infiles->ampfile2, linelen, nlines,
                  tileparams, sizeof(float *), sizeof(float));
    } else {
      fflush(NULL);
 ...
 */
int ReadCorrelation(float ***corrptr, infileT *infiles, long linelen,
                    long nlines, tileparamT *tileparams) {
  ...
  if (infiles->corrfileformat == ALT_SAMPLE_DATA) {
    ReadAltSampFile(&dummy, &corr, infiles->corrfile, linelen, nlines,
                    tileparams);
 ...
 * data.
 */
int ReadAltLineFile(float ***mag, float ***phase, char *alfile, long linelen,
                    long nlines, tileparamT *tileparams) {
  ...
  /* read the data */
  fseek(fp,
        (tileparams->firstrow * 2 * linelen + tileparams->firstcol) *
            sizeof(float),
        SEEK_CUR);
 ...
 * data.
 */
int ReadAltLineFilePhase(float ***phase, char *alfile, long linelen,
                         long nlines, tileparamT *tileparams) {
  ...
  /* read the phase data */
  fseek(fp,
        (tileparams->firstrow * 2 * linelen + linelen + tileparams->firstcol) *
            sizeof(float),
        SEEK_CUR);
 ...
 * [0,2pi).
 */
int ReadComplexFile(float ***mag, float ***phase, char *rifile, long linelen,
                    long nlines, tileparamT *tileparams) {
  ...
  fseek(fp,
        (tileparams->firstrow * linelen + tileparams->firstcol) * 2 *
            sizeof(float),
        SEEK_CUR);
  ...
  for (row = 0; row < nrow; row++) {
    ...
    for (col = 0; col < ncol; col++) {
      ...
      if (inpline[2 * col + 1] == 0 && inpline[2 * col] == 0) {
        (*phase)[row][col] = 0;
      } else if (!IsFinite((*phase)[row][col] =
                               atan2(inpline[2 * col + 1], inpline[2 * col]))) {
        (*phase)[row][col] = 0;
...
int Read2DArray(void ***arr, char *infile, long linelen, long nlines,
  ...
  /* read the data */
  fseek(fp, (linelen * tileparams->firstrow + tileparams->firstcol) * elsize,
        SEEK_CUR);
 ...
 */
int ReadAltSampFile(float ***arr1, float ***arr2, char *infile, long linelen,
                    long nlines, tileparamT *tileparams) {
...
int Read2DRowColFile(void ***arr, char *filename, long linelen, long nlines,
  ...
  if (2 * linelen * nlines - nlines - linelen != nel || (filelen % size)) {
    fflush(NULL);
    fprintf(sp0, "File %s element size %ld", filename, size);
    fprintf(sp0, "File %s wrong size %ld (%ld elements expected)\nAbort\n",
            filename, nel, 2 * linelen * nlines - nlines - linelen);
    exit(ABNORMAL_EXIT);
  }
  ...
  /* read arrays */
  fseek(fp, (linelen * tileparams->firstrow + tileparams->firstcol) * size,
        SEEK_SET);
  ...
  fseek(fp,
        (linelen * (nlines - 1) + (linelen - 1) * tileparams->firstrow +
         tileparams->firstcol) *
            size,
        SEEK_SET);
  for (row = nrow - 1; row < 2 * nrow - 1; row++) {
 ...
 * in the RowCol file).
 */
int Read2DRowColFileRows(void ***arr, char *filename, long linelen, long nlines,
                         tileparamT *tileparams, size_t size) {
...
int ChildResetStreamPointers(pid_t pid, long tilerow, long tilecol,
  ...
  fflush(NULL);
  sprintf(logfile, "%s/%s%ld_%ld", params->tiledir, LOGFILEROOT, tilerow,
          tilecol);
 ...
 */
int DumpIncrCostFiles(incrcostT **incrcosts, long iincrcostfile, long nflow,
                      long nrow, long ncol) {
  ...
  Write2DRowColArray((void **)tempcosts, incrcostfile, nrow, ncol,
                     sizeof(short));
  for (row = 0; row < 2 * nrow - 1; row++) {
    if (row < nrow - 1) {
      maxcol = ncol;
...
int MakeTileDir(paramT *params, outfileT *outfiles) {
  ...
  if (!strlen(params->tiledir)) {
    ...
    sprintf(params->tiledir, "%s%s%ld", path, TMPTILEDIRROOT,
            params->parentpid);
  }
...
int SetTileInitOutfile(char *outfile, long pid) {
  ...
  if (!stat(outfile, statbuf)) {
    fprintf(sp0, "ERROR: refusing to write tile init to existing file %s\n",
            outfile);
    exit(ABNORMAL_EXIT);
  }
