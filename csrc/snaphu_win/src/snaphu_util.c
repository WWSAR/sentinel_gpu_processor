...
static int IsFalse(char *str);
static double ModDiff(double f1, double f2);
...
int CalcWrappedRangeDiffs(float **dpsi, float **avgdpsi, float **wrappedphase,
  ...
  for (row = 0; row < nrow; row++) {
    for (col = 0; col < ncol - 1; col++) {
      dpsi[row][col] =
          (wrappedphase[row][col + 1] - wrappedphase[row][col]) / TWOPI;
      if (dpsi[row][col] >= 0.5) {
        dpsi[row][col] -= 1.0;
  ...
  paddpsi =
      MirrorPad(dpsi, nrow, ncol - 1, (kperpdpsi - 1) / 2, (kpardpsi - 1) / 2);
  if (paddpsi == dpsi) {
    fflush(NULL);
...
int CalcWrappedAzDiffs(float **dpsi, float **avgdpsi, float **wrappedphase,
  ...
  for (row = 0; row < nrow - 1; row++) {
    for (col = 0; col < ncol; col++) {
      dpsi[row][col] =
          (wrappedphase[row][col] - wrappedphase[row + 1][col]) / TWOPI;
      if (dpsi[row][col] >= 0.5) {
        dpsi[row][col] -= 1.0;
      } else if (dpsi[row][col] < -0.5) {
        dpsi[row][col] += 1.0;
  ...
  paddpsi =
      MirrorPad(dpsi, nrow - 1, ncol, (kpardpsi - 1) / 2, (kperpdpsi - 1) / 2);
  if (paddpsi == dpsi) {
    fflush(NULL);
    fprintf(sp0, "Wrapped-gradient averaging box too large "
                 "for input array size\nAbort\n");
...
int CycleResidue(float **phase, signed char **residue, int nrow, int ncol) {
  ...
  for (row = 0; row < nrow - 1; row++) {
    for (col = 0; col < ncol - 1; col++) {
      residue[row][col] =
          (signed char)LRound((coldiff[row][col] + rowdiff[row][col + 1] -
                               coldiff[row + 1][col] - rowdiff[row][col]) /
                              TWOPI);
    }
  }
...
int NodeResidue(float **wphase, long row, long col) {
  ...
  /* compute residue */
  residue =
      (int)LRound((ModDiff(wphase[row][col + 1], wphase[row][col]) +
                   ModDiff(wphase[row + 1][col + 1], wphase[row][col + 1]) +
                   ModDiff(wphase[row + 1][col], wphase[row + 1][col + 1]) +
                   ModDiff(wphase[row][col], wphase[row + 1][col])) /
                  TWOPI);
...
int CalcFlow(float **phase, short ***flowsptr, long nrow, long ncol) {
  ...
  for (row = 0; row < nrow - 1; row++) {
    for (col = 0; col < ncol; col++) {
      (*flowsptr)[row][col] =
          (short)LRound((phase[row][col] - phase[row + 1][col]) / TWOPI);
    }
  }
  ...
  for (row = 0; row < nrow; row++) {
    for (col = 0; col < ncol - 1; col++) {
      (*flowsptr)[nrow - 1 + row][col] =
          (short)LRound((phase[row][col + 1] - phase[row][col]) / TWOPI);
    }
  }
...
int IntegratePhase(float **psi, float **phi, short **flows, long nrow,
  ...
  for (row = 1; row < nrow; row++) {
    for (col = 0; col < ncol; col++) {
      phi[row][col] =
          phi[row - 1][col] + (ModDiff(psi[row][col], psi[row - 1][col]) -
...
void **Get2DRowColZeroMem(long nrow, long ncol, int psize, size_t size) {
  ...
  for (row = nrow - 1; row < 2 * nrow - 1; row++) {
    arr[row] = &((
        (char *)baseptr)[((nrow - 1) * ncol + (row - (nrow - 1)) * (ncol - 1)) *
                         size]);
  }
 ...
 */
long LRound(double a) { return ((long)rint(a)); }
...
float LinInterp1D(float *arr, double index, long nelem) {
  ...
  if (intpart < 0) {
    ...
    return (((1 - fracpart) * arr[intpart] + fracpart * arr[intpart + 1]) /
            2.0);
  }
}
...
float **MirrorPad(float **array1, long nrow, long ncol, long krow, long kcol) {
  ...
  /* get memory */
  array2 = (float **)Get2DMem(nrow + 2 * krow, ncol + 2 * kcol, sizeof(float *),
                              sizeof(float));
  ...
  for (row = krow; row < nrow + krow; row++) {
    for (col = 0; col < kcol; col++) {
      ...
      array2[row][ncol + kcol + col] = array2[row][ncol + kcol - 2 - col];
    }
  }
  for (col = kcol; col < ncol + kcol; col++) {
    for (row = 0; row < krow; row++) {
      array2[row][col] = array2[2 * krow - row][col];
      array2[nrow + krow + row][col] = array2[nrow + krow - 2 - row][col];
    }
  }
