...
#ifdef _WIN32
#define DUMP_PATH "./" /* default location for writing dumps on Windows */
#else
...
#ifdef _WIN32
#define TILEDIRMODE 0 /* mode ignored by _mkdir on Windows */
#else
...
#define OPTIONSHELPFULL                                                        \
  ...
  "  --tile <nrow> <ncol> <rowovrlp> <colovrlp>  unwrap as nrow x ncol "       \
  "tiles\n"                                                                    \
...
typedef struct paramST {
  ...
  double cstd1, cstd2,
      cstd3;           /* for calculating correlation power given nlooks */
...
int MSTInitFlows(float **wrappedphase, short ***flowsptr, short **mstcosts,
                 long nrow, long ncol, nodeT ***nodes, nodeT *ground,
                 long maxflow);
...
int CalcWrappedAzDiffs(float **dpsi, float **avgdpsi, float **wrappedphase,
                       long kperpdpsi, long kpardpsi, long nrow, long ncol);
int CycleResidue(float **phase, signed char **residue, int nrow, int ncol);
int NodeResidue(float **wphase, long row, long col);
int CalcFlow(float **phase, short ***flowsptr, long nrow, long ncol);
int IntegratePhase(float **psi, float **phi, short **flows, long nrow,
                   long ncol);
float **ExtractFlow(float **unwrappedphase, short ***flowsptr, long nrow,
                    long ncol);
...
float LinInterp2D(float **arr, double rowind, double colind, long nrow,
                  long ncol);
int Despeckle(float **mag, float ***ei, long nrow, long ncol);
float **MirrorPad(float **array1, long nrow, long ncol, long krow, long kcol);
int BoxCarAvg(float **avgarr, float **padarr, long nrow, long ncol, long krow,
              long kcol);
char *StrNCopy(char *dest, const char *src, size_t n);
int FlattenWrappedPhase(float **wrappedphase, float **unwrappedest, long nrow,
                        long ncol);
...
extern void (*CalcCost)(void **, long, long, long, long, long, paramT *, long *,
                        long *);
