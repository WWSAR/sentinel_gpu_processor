...
static void AddNewNode(nodeT *from, nodeT *to, long arcdir, bucketT *bkts,
                       long nflow, incrcostT **incrcosts, long arcrow,
                       long arccol, paramT *params);
static void CheckArcReducedCost(nodeT *from, nodeT *to, nodeT *apex,
                                long arcrow, long arccol, long arcdir,
                                candidateT **candidatebagptr,
                                long *candidatebagnextptr,
                                long *candidatebagsizeptr,
                                incrcostT **incrcosts,
                                signed char **iscandidate, paramT *params);
static nodeT *InitBoundary(nodeT *source, nodeT **nodes, boundaryT *boundary,
                           nodesuppT **nodesupp, float **mag, nodeT *ground,
                           long ngroundarcs, long nrow, long ncol,
                           paramT *params, long *nconnectedptr);
static long CheckBoundary(nodeT **nodes, float **mag, nodeT *ground,
                          long ngroundarcs, boundaryT *boundary, long nrow,
                          long ncol, paramT *params, nodeT *start);
static int IsRegionEdgeArc(float **mag, long arcrow, long arccol, long nrow,
                           long ncol);
static int IsRegionInteriorArc(float **mag, long arcrow, long arccol, long nrow,
                               long ncol);
static int IsRegionArc(float **mag, long arcrow, long arccol, long nrow,
                       long ncol);
static int IsRegionEdgeNode(float **mag, long row, long col, long nrow,
                            long ncol);
static int CleanUpBoundaryNodes(nodeT *source, boundaryT *boundary, float **mag,
                                nodeT **nodes, nodeT *ground, long nrow,
                                long ncol, long ngroundarcs,
                                nodesuppT **nodesupp);
static int DischargeBoundary(nodeT **nodes, nodeT *ground, boundaryT *boundary,
                             nodesuppT **nodesupp, short **flows,
                             signed char **iscandidate, float **mag,
                             float **wrappedphase, long ngroundarcs, long nrow,
                             long ncol);
static int InitTree(nodeT *source, nodeT **nodes, boundaryT *boundary,
                    nodesuppT **nodesupp, nodeT *ground, long ngroundarcs,
                    bucketT *bkts, long nflow, incrcostT **incrcosts, long nrow,
                    long ncol, paramT *params);
static nodeT *FindApex(nodeT *from, nodeT *to);
static int CandidateCompare(const void *c1, const void *c2);
static nodeT *NeighborNodeGrid(nodeT *node1, long arcnum, long *upperarcnumptr,
                               nodeT **nodes, nodeT *ground, long *arcrowptr,
                               long *arccolptr, long *arcdirptr, long nrow,
                               long ncol, boundaryT *boundary,
                               nodesuppT **nodesupp);
static inline long GetArcNumLims(long fromrow, long *upperarcnumptr,
                                 long ngroundarcs, boundaryT *boundary);
static nodeT *NeighborNodeNonGrid(nodeT *node1, long arcnum,
                                  long *upperarcnumptr, nodeT **nodes,
                                  nodeT *ground, long *arcrowptr,
                                  long *arccolptr, long *arcdirptr, long nrow,
                                  long ncol, boundaryT *boundary,
                                  nodesuppT **nodesupp);
static void GetArcGrid(nodeT *from, nodeT *to, long *arcrow, long *arccol,
                       long *arcdir, long nrow, long ncol, nodeT **nodes,
                       nodesuppT **nodesupp);
static void GetArcNonGrid(nodeT *from, nodeT *to, long *arcrow, long *arccol,
                          long *arcdir, long nrow, long ncol, nodeT **nodes,
                          nodesuppT **nodesupp);
static void NonDegenUpdateChildren(nodeT *startnode, nodeT *lastnode,
                                   nodeT *nextonpath, long dgroup,
                                   long ngroundarcs, long nflow, nodeT **nodes,
                                   nodesuppT **nodesupp, nodeT *ground,
                                   boundaryT *boundary, nodeT ***apexes,
                                   incrcostT **incrcosts, long nrow, long ncol,
                                   paramT *params);
static long PruneTree(nodeT *source, nodeT **nodes, nodeT *ground,
                      boundaryT *boundary, nodesuppT **nodesupp,
                      incrcostT **incrcosts, short **flows, long ngroundarcs,
                      long prunecostthresh, long nrow, long ncol);
static int CheckLeaf(nodeT *node1, nodeT **nodes, nodeT *ground,
                     boundaryT *boundary, nodesuppT **nodesupp,
                     incrcostT **incrcosts, short **flows, long ngroundarcs,
                     long nrow, long ncol, long prunecostthresh);
static int GridNodeMaskStatus(long row, long col, float **mag);
static int GroundMaskStatus(long nrow, long ncol, float **mag);
static int InitBuckets(bucketT *bkts, nodeT *source, long nbuckets);
static nodeT *MinOutCostNode(bucketT *bkts);
static nodeT *SelectConnNodeSource(nodeT **nodes, float **mag, nodeT *ground,
                                   long ngroundarcs, long nrow, long ncol,
                                   paramT *params, nodeT *start,
                                   long *nconnectedptr);
static long ScanRegion(nodeT *start, nodeT **nodes, float **mag, nodeT *ground,
                       long ngroundarcs, long nrow, long ncol,
                       int groupsetting);
static short GetCost(incrcostT **incrcosts, long arcrow, long arccol,
                     long arcdir);
static void SolveMST(nodeT **nodes, nodeT *source, nodeT *ground, bucketT *bkts,
                     short **mstcosts, signed char **residue,
                     signed char **arcstatus, long nrow, long ncol);
static long DischargeTree(nodeT *source, short **mstcosts, short **flows,
                          signed char **residue, signed char **arcstatus,
                          nodeT **nodes, nodeT *ground, long nrow, long ncol);
static signed char ClipFlow(signed char **residue, short **flows,
...
long TreeSolve(nodeT **nodes, nodesuppT **nodesupp, nodeT *ground,
               ...
               float **mag, float **wrappedphase, char *outfile, long nnoderow,
               int *nnodesperrow, long narcrow, int *narcsperrow, long nrow,
               long ncol, outfileT *outfiles, long nconnected, paramT *params) {
  ...
  source = InitBoundary(source, nodes, boundary, nodesupp, mag, ground,
                        ngroundarcs, nrow, ncol, params, &nconnected);
  ...
  ;
  fprintf(sp3, "Treesize: %-10ld Pivots: %-11ld Improvements: %-11ld", treesize,
          ipivots, inondegen);
  ...
  while (treesize < nconnected) {
    ...
    while (nnewnodes < maxnewnodes && treesize < nconnected) {
      ...
      while (arcnum < upperarcnum) {
        ...
        to = NeighborNode(from, ++arcnum, &upperarcnum, nodes, ground, &arcrow,
                          &arccol, &arcdir, nrow, ncol, boundary, nodesupp);
        ...
        if (to->group > 0) {
          ...
          AddNewNode(from, to, arcdir, bkts, nflow, incrcosts, arcrow, arccol,
                     params);
        }
      }
      nnewnodes++;
    ...
    while (candidatebagnext) {
      ...
      if (dumpresults_global) {
        ...
        unwrappedphase =
            (float **)Get2DMem(nrow, ncol, sizeof(float *), sizeof(float));
        IntegratePhase(wrappedphase, unwrappedphase, flows, nrow, ncol);
        FlipPhaseArraySign(unwrappedphase, params, nrow, ncol);
        WriteOutputFile(mag, unwrappedphase, outfiles->outfile, outfiles, nrow,
                        ncol);
        if (requestedstop_global) {
          fflush(NULL);
      ...
      */
      ...
      for (i = 0; i < candidatelistlen; i++) {
        ...
        outcostto = from->outcost + GetCost(incrcosts, arcrow, arccol, arcdir);
        cyclecost = outcostto + to->incost - apexes[arcrow][arccol]->outcost -
        ...
        if (!((outcostto < to->outcost) || (cyclecost < 0))) {
          ...
          arcdir = -arcdir;
          outcostto =
              from->outcost + GetCost(incrcosts, arcrow, arccol, arcdir);
          cyclecost = outcostto + to->incost - apexes[arcrow][arccol]->outcost -
                      apexes[arcrow][arccol]->incost;
        }
        ...
        if ((outcostto < to->outcost) || (cyclecost < 0)) {
          ...
          if (cyclecost < 0) {
            ...
            while (TRUE) {
              ...
              ReCalcCost(costs, incrcosts, flows[arcrow][arccol], arcrow,
                         arccol, nflow, nrow, params);
              violation = GetCost(incrcosts, arcrow, arccol, arcdir);
              if (node1->level > node2->level) {
                while (node1->level != node2->level) {
                  GetArc(node1->pred, node1, &arcrow1, &arccol1, &arcdir1, nrow,
                         ncol, nodes, nodesupp);
                  flows[arcrow1][arccol1] += (arcdir1 * nflow);
                  ReCalcCost(costs, incrcosts, flows[arcrow1][arccol1], arcrow1,
                             arccol1, nflow, nrow, params);
                  if (leavingchild == NULL && !flows[arcrow1][arccol1]) {
                    leavingchild = node1;
                  }
                ...
                while (node1->level != node2->level) {
                  GetArc(node2->pred, node2, &arcrow2, &arccol2, &arcdir2, nrow,
                         ncol, nodes, nodesupp);
                  flows[arcrow2][arccol2] -= (arcdir2 * nflow);
                  ReCalcCost(costs, incrcosts, flows[arcrow2][arccol2], arcrow2,
                             arccol2, nflow, nrow, params);
              ...
              while (node1 != node2) {
                ...
                if (!flows[arcrow2][arccol2]) {
                  leavingchild = node2;
                  fromside = FALSE;
                } else if (leavingchild == NULL && !flows[arcrow1][arccol1]) {
                  leavingchild = node1;
                }
                node1->group = groupcounter + 1;
                node2->group = groupcounter;
          ...
          if (cyclecost < 0) {
            ...
            firstfromnode = NULL;
            firsttonode = NULL;
            arcnum = GetArcNumLims(cycleapex->row, &upperarcnum, ngroundarcs,
                                   boundary);
            while (arcnum < upperarcnum) {
              tempnode1 = NeighborNode(cycleapex, ++arcnum, &upperarcnum, nodes,
                                       ground, &arcrow, &arccol, &arcdir, nrow,
                                       ncol, boundary, nodesupp);
            ...
            if (firsttonode != NULL) {
              NonDegenUpdateChildren(cycleapex, leavingparent, firsttonode, 0,
                                     ngroundarcs, nflow, nodes, nodesupp,
                                     ground, boundary, apexes, incrcosts, nrow,
                                     ncol, params);
            }
            if (firstfromnode != NULL) {
          ...
          if (leavingchild == NULL) {
            ...
            while (oldmntpt != leavingparent) {
              ...
              oldmntpt = root->pred;
              root->pred = mntpt;
              GetArc(mntpt, root, &arcrow, &arccol, &arcdir, nrow, ncol, nodes,
                     nodesupp);
            ...
            GetArc(from, to, &arcrow, &arccol, &arcdir, nrow, ncol, nodes,
                   nodesupp);
            apexes[arcrow][arccol] = NULL;
            GetArc(leavingparent, leavingchild, &arcrow, &arccol, &arcdir, nrow,
                   ncol, nodes, nodesupp);
            apexes[arcrow][arccol] = cycleapex;
            ...
            while (TRUE) {
              ...
              arcnum = GetArcNumLims(node1->row, &upperarcnum, ngroundarcs,
                                     boundary);
              while (arcnum < upperarcnum) {
                node2 = NeighborNode(node1, ++arcnum, &upperarcnum, nodes,
                                     ground, &arcrow, &arccol, &arcdir, nrow,
                                     ncol, boundary, nodesupp);
                ...
                if (node2->group > 0) {
                  ...
                  if (node2->group < node1->group &&
                    ...
                    if (node2->group >= apexlistbase) {
                      ...
                      apexes[arcrow][arccol] =
                          apexlist[node2->group - apexlistbase];
          ...
          if (cyclecost < 0) {
            ...
            while (TRUE) {
              ...
              if (firstfromnode != NULL && firstfromnode->pred == cycleapex) {
                node1 = firstfromnode;
                firstfromnode = NULL;
              } else if (firsttonode != NULL &&
                         firsttonode->pred == cycleapex) {
                node1 = firsttonode;
                firsttonode = NULL;
              } else {
                break;
    ...
    if (!((++nmajor) % nmajorprune)) {
      npruned += PruneTree(source, nodes, ground, boundary, nodesupp, incrcosts,
                           flows, ngroundarcs, prunecostthresh, nrow, ncol);
    }
  ...
  while (node1 != source) {
    if (node1->pred->level != node1->level - 1) {
      printf("Error detected: row %d, col%d, level %d "
             ...
             node1->row, node1->col, node1->level, node1->pred->row,
             node1->pred->col, node1->pred->level);
    }
    node1 = node1->next;
  }
  ...
  DischargeBoundary(nodes, ground, boundary, nodesupp, flows, iscandidate, mag,
                    wrappedphase, ngroundarcs, nrow, ncol);
  ...
  CleanUpBoundaryNodes(source, boundary, mag, nodes, ground, nrow, ncol,
                       ngroundarcs, nodesupp);
...
static void AddNewNode(nodeT *from, nodeT *to, long arcdir, bucketT *bkts,
  ...
  newoutcost = from->outcost + GetCost(incrcosts, arcrow, arccol, arcdir);
...
static nodeT *InitBoundary(nodeT *source, nodeT **nodes, boundaryT *boundary,
  ...
  nconnected =
      ScanRegion(source, nodes, mag, ground, ngroundarcs, nrow, ncol, MASKED);
  ...
  while (TRUE) {
    ...
    while (arcnum < upperarcnum) {
      ...
      to = NeighborNode(from, ++arcnum, &upperarcnum, nodes, ground, &arcrow,
                        &arccol, &arcdir, nrow, ncol, NULL, nodesupp);
  ...
  for (k = 0; k < nlist; k++) {
    ...
    if (nodelist[k]->row != GROUNDROW) {
      ...
      while (arcnum < upperarcnum) {
        ...
        /* get neighbor */
        from =
            NeighborNode(nodelist[k], ++arcnum, &upperarcnum, nodes, ground,
        ...
        isinteriornode =
            (IsRegionInteriorArc(mag, arcrow, arccol, nrow, ncol) &&
             from->group != MASKED && from->level != BOUNDARYLEVEL);
        if (isinteriornode) {
          ninteriorneighbor++;
        }
      ...
      if (iseligible && ninteriorneighbor > 0) {
        ...
        if (++boundary->nboundary > nboundarymem) {
          nboundarymem += NLISTMEMINCR;
          boundarylist =
              (nodeT **)ReAlloc(boundarylist, nboundarymem * sizeof(nodeT *));
        }
  ...
  for (k = 0; k < boundary->nboundary; k++) {
    ...
    arcnum =
        GetArcNumLims(boundarylist[k]->row, &upperarcnum, ngroundarcs, NULL);
    while (arcnum < upperarcnum) {
  ...
  boundary->neighborlist = (neighborT *)ReAlloc(
      neighborlist, (boundary->nneighbor * sizeof(neighborT)));
  boundary->boundarylist =
      (nodeT **)ReAlloc(boundarylist, (boundary->nboundary * sizeof(nodeT *)));
   ...
   */
  nconnected = CheckBoundary(nodes, mag, ground, ngroundarcs, boundary, nrow,
                             ncol, params, boundary->node);
...
static long CheckBoundary(nodeT **nodes, float **mag, nodeT *ground,
  ...
  while (node1 != NULL) {
    ...
    while (arcnum < upperarcnum) {
      node2 =
          NeighborNode(node1, ++arcnum, &upperarcnum, nodes, ground, &arcrow,
                       &arccol, &arcdir, nrow, ncol, boundary, nodesupp);
  ...
  if (nboundaryarc != boundary->nneighbor) {
    fflush(NULL);
    fprintf(sp0, "ERROR: inconsistent num neighbor nodes in CheckBoundary()\n");
    exit(ABNORMAL_EXIT);
  }
  if (nboundarynode != 1) {
    fflush(NULL);
...
static int IsRegionEdgeNode(float **mag, long row, long col, long nrow,
  ...
  onezeromag = FALSE;
  onenonzeromag = FALSE;
  if (mag[row][col] == 0 || mag[row + 1][col] == 0 || mag[row][col + 1] == 0 ||
      mag[row + 1][col + 1] == 0) {
    onezeromag = TRUE;
  }
  if (mag[row][col] != 0 || mag[row + 1][col] != 0 || mag[row][col + 1] != 0 ||
...
static int CleanUpBoundaryNodes(nodeT *source, boundaryT *boundary, float **mag,
  ...
  nconnected =
      ScanRegion(start, nodes, mag, ground, ngroundarcs, nrow, ncol, 0);
...
static int DischargeBoundary(nodeT **nodes, nodeT *ground, boundaryT *boundary,
  ...
  if (nodesupp != NULL || boundary == NULL || boundary->nboundary == 0 ||
      boundary->nneighbor == 0) {
    return (0);
  }
  ...
  while (TRUE) {
    ...
    if ((--iscandidate[row][col]) == -2) {
      ...
      fromrow = from->row;
      fromcol = from->col;
      surplus = (flows[fromrow][fromcol] - flows[fromrow][fromcol + 1] +
...
static int InitTree(nodeT *source, nodeT **nodes, boundaryT *boundary,
  ...
  while (arcnum < upperarcnum) {
    ...
    to = NeighborNode(source, ++arcnum, &upperarcnum, nodes, ground, &arcrow,
                      &arccol, &arcdir, nrow, ncol, boundary, nodesupp);

    /* add to node to bucket */
    if (to->group != PRUNED && to->group != MASKED) {
      AddNewNode(source, to, arcdir, bkts, nflow, incrcosts, arcrow, arccol,
                 params);
    }
  }
  ...
  if (fromcol == tocol - 1) { /* normal arcs (neither endpoint ground) */
    ...
    *arcdir = -1;
    ...
    if (tocol < ncol - 2 && nodes[torow][tocol + 1].group == BOUNDARYPTR) {
      ...
      *arccol = tocol;
      *arcdir = 1;
    } else if (torow < nrow - 2 &&
               nodes[torow + 1][tocol].group == BOUNDARYPTR) {
    ...
    if (fromcol < ncol - 2 &&
        nodes[fromrow][fromcol + 1].group == BOUNDARYPTR) {
      *arcrow = fromrow;
      *arccol = fromcol + 1;
      *arcdir = 1;
    } else if (fromcol > 0 &&
      ...
      *arcdir = -1;
               ...
               nodes[fromrow + 1][fromcol].group == BOUNDARYPTR) {
      *arcrow = fromrow + 1 + nrow - 1;
      *arccol = fromcol;
      *arcdir = 1;
    } else {
...
static void NonDegenUpdateChildren(nodeT *startnode, nodeT *lastnode,
  ...
  while (node1 != lastnode) {
    ...
    node2 = nextonpath;
    GetArc(node2->pred, node2, &arcrow, &arccol, &arcdir, nrow, ncol, nodes,
           nodesupp);
...
static int CheckLeaf(nodeT *node1, nodeT **nodes, nodeT *ground,
  ...
  arcnum = GetArcNumLims(node1->row, &upperarcnum, ngroundarcs, boundary);
  while (arcnum < upperarcnum) {
    node2 = NeighborNode(node1, ++arcnum, &upperarcnum, nodes, ground, &arcrow,
                         &arccol, &arcdir, nrow, ncol, boundary, nodesupp);
...
int InitNetwork(short **flows, long *ngroundarcsptr, long *ncycleptr,
  ...
  if (ground != NULL && *nodesptr == NULL) {
    *nodesptr =
        (nodeT **)Get2DMem(nrow - 1, ncol - 1, sizeof(nodeT *), sizeof(nodeT));
  ...
  *candidatelistptr = MAlloc(*candidatelistsizeptr * sizeof(candidateT));
  if (ground != NULL) {
    ...
    *mostflowptr = Short2DRowColAbsMax(flows, nrow, ncol);
    ...
    if (ncol > 2) {
      *ngroundarcsptr =
          2 * (nrow + ncol - 2) - 4; /* don't include corner column arcs */
    } else {
      *ngroundarcsptr = 2 * (nrow + ncol - 2) - 2;
    }
    *iscandidateptr = (signed char **)Get2DRowColMem(
        nrow, ncol, sizeof(signed char *), sizeof(signed char));
  ...
  *bktsptr = MAlloc(sizeof(bucketT));
  if (ground != NULL) {
    (*bktsptr)->minind =
        -LRound((params->maxcost + 1) * (nrow + ncol) * NEGBUCKETFRACTION);
    (*bktsptr)->maxind =
        LRound((params->maxcost + 1) * (nrow + ncol) * POSBUCKETFRACTION);
  } else {
    (*bktsptr)->minind =
        -LRound((params->maxcost + 1) * (nrow)*NEGBUCKETFRACTION);
    (*bktsptr)->maxind =
        LRound((params->maxcost + 1) * (nrow)*POSBUCKETFRACTION);
  }
  ...
  *iincrcostfileptr = 0;
  if (ground != NULL) {
    (*incrcostsptr) = (incrcostT **)Get2DRowColMem(
        nrow, ncol, sizeof(incrcostT *), sizeof(incrcostT));
  }
 ...
 */
int MaskNodes(long nrow, long ncol, nodeT **nodes, nodeT *ground, float **mag) {
...
static int GridNodeMaskStatus(long row, long col, float **mag) {
  ...
  if (mag[row][col] || mag[row][col + 1] || mag[row + 1][col] ||
      mag[row + 1][col + 1]) {
    return (0);
  }
  return (MASKED);
...
long MaxNonMaskFlow(short **flows, float **mag, long nrow, long ncol) {
  ...
  for (row = nrow - 1; row < 2 * nrow - 1; row++) {
    for (col = 0; col < ncol - 1; col++) {
      flowvalue = labs((long)flows[row][col]);
      if (flowvalue > mostflow && mag[row - nrow + 1][col] > 0 &&
          mag[row - nrow + 1][col + 1] > 0) {
        mostflow = flowvalue;
      }
    }
  }
...
long SelectSources(nodeT **nodes, float **mag, nodeT *ground, long nflow,
                   ...
                   paramT *params, nodeT ***sourcelistptr,
                   long **nconnectedarrptr) {
  ...
  for (row = 0; row < nrow - 1; row++) {
    for (col = 0; col < ncol - 1; col++) {
      if (nodes[row][col].group != MASKED &&
          nodes[row][col].group != BOUNDARYPTR) {
        nodes[row][col].group = 0;
      }
  ...
  source = SelectConnNodeSource(nodes, mag, ground, ngroundarcs, nrow, ncol,
                                params, ground, &nconnected);
  if (source != NULL) {
  ...
  for (row = 0; row < nrow - 1; row++) {
    for (col = 0; col < ncol - 1; col++) {
      ...
      source = SelectConnNodeSource(nodes, mag, ground, ngroundarcs, nrow, ncol,
                                    params, &nodes[row][col], &nconnected);
      if (source != NULL) {
...
static nodeT *SelectConnNodeSource(nodeT **nodes, float **mag, nodeT *ground,
  ...
  nconnected =
      ScanRegion(start, nodes, mag, ground, ngroundarcs, nrow, ncol, ONTREE);
 ...
 */
long ReCalcCost(void **costs, incrcostT **incrcosts, long flow, long arcrow,
                long arccol, long nflow, long nrow, paramT *params) {

  long poscost, negcost, iclipped;

  /* calculate new positive and negative nflow costs, as long ints */
  CalcCost(costs, flow, arcrow, arccol, nflow, nrow, params, &poscost,
           &negcost);
...
int SetupIncrFlowCosts(void **costs, incrcostT **incrcosts, short **flows,
                       long nflow, long nrow, long narcrow, int *narcsperrow,
                       paramT *params) {
  ...
  for (arcrow = 0; arcrow < narcrow; arcrow++) {
    ...
    for (arccol = 0; arccol < narcsperrow[arcrow]; arccol++) {
      ...
      /* calculate new positive and negative nflow costs, as long ints */
      iclipped += ReCalcCost(costs, incrcosts, flows[arcrow][arccol], arcrow,
                             arccol, nflow, nrow, params);
    }
  }
 ...
 * algorithm.
 */
int MSTInitFlows(float **wrappedphase, short ***flowsptr, short **mstcosts,
                 long nrow, long ncol, nodeT ***nodesptr, nodeT *ground,
                 long maxflow) {
  ...
  // Write2DRowColArray(mstcosts,"mstcosts",nrow,ncol,sizeof(mstcosts[0][0]));
  ...
  *nodesptr =
  ...
  for (row = 0; row < 2 * nrow - 1; row++) {
    ...
    for (col = 0; col < i; col++) {
      if (mstcosts[row][col] > maxcost && !((row == nrow - 1 || 2 * nrow - 2) &&
                                            (col == 0 || col == ncol - 2))) {
  ...
  (*flowsptr) =
      (short **)Get2DRowColZeroMem(nrow, ncol, sizeof(short *), sizeof(short));
  flows = *flowsptr;
  ...
  while (TRUE) {
    ...
    DischargeTree(source, mstcosts, flows, residue, arcstatus, *nodesptr,
                  ground, nrow, ncol);

    // Write2DRowColArray(*flowsptr,"flows",nrow,ncol,sizeof(flows[0][0]));
  ...
  while ((from = ClosestNode(bkts)) != NULL) {
    ...
    if (((fromrow != GROUNDROW && residue[fromrow][fromcol]) ||
      ...
      while (TRUE) {
        ...
        GetArc(pathfrom, pathto, &arcrow, &arccol, &arcdir, nrow, ncol, nodes,
               nodesupp);
