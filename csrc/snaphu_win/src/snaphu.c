...
void (*CalcCost)(void **, long, long, long, long, long, paramT *, long *,
                 long *) = NULL;
long (*EvalCost)(void **, short **, long, long, long, paramT *) = NULL;

/* static (local) function prototypes */
static int Unwrap(infileT *infiles, outfileT *outfiles, paramT *params,
                  long linelen, long nlines);
static int UnwrapTile(infileT *infiles, outfileT *outfiles, paramT *params,
...
static int Unwrap(infileT *infiles, outfileT *outfiles, paramT *params,
  ...
  for (optiter = 0; optiter < noptiter; optiter++) {
    ...
    if (ntilerow == 1 && ntilecol == 1) {
      ...
      UnwrapTile(iterinfiles, iteroutfiles, iterparams, tileparams, nlines,
                 linelen);
      ...
      if (!iterparams->assembleonly) {
        ...
        if (nthreads > 1) {
          ...
          nchildren = 0;
          sleepinterval =
              (long)ceil(nlines * linelen / ((double)(ntilerow * ntilecol)) *
                         SECONDSPERPIXEL);
          ...
          while (TRUE) {
            ...
            if (nchildren < nthreads && nexttilerow < ntilerow) {
              ...
              if (pid < 0) {
                ...
                SetupTile(nlines, linelen, iterparams, tileparams, iteroutfiles,
                          tileoutfiles, nexttilerow, nexttilecol);
              ...
              if (!(WIFEXITED(childstatus)) ||
                ...
                fflush(NULL);
                fprintf(sp0,
                        "Unexpected or abnormal exit of child process %ld\n"
                        "Abort\n",
                        (long)pid);
...
#else  /* _WIN32 */
        /* Windows: fork() not available, always use sequential processing */
        if (nthreads > 1) {
          fprintf(sp1, "Parallel tiling not supported on Windows; "
...
static int UnwrapTile(infileT *infiles, outfileT *outfiles, paramT *params,
  ...
  ReadInputFile(infiles, &mag, &wrappedphase, &flows, linelen, nlines, params,
                tileparams);
  ...
  if (strlen(infiles->estfile)) {
    ReadUnwrappedEstimateFile(&unwrappedest, infiles, linelen, nlines, params,
                              tileparams);
  ...
  BuildCostArrays(&costs, &mstcosts, mag, wrappedphase, unwrappedest, linelen,
                  nlines, nrow, ncol, params, tileparams, infiles, outfiles);
  ...
  if (!params->unwrapped) {
    ...
    if (params->initmethod == MSTINIT) {
      ...
      MSTInitFlows(wrappedphase, &flows, mstcosts, nrow, ncol, &nodes, ground,
                   params->initmaxflow);
    ...
    if (params->initonly || strlen(outfiles->initfile)) {
      ...
      if (params->initonly) {
        ...
        WriteOutputFile(mag, unwrappedphase, outfiles->initfile, outfiles, nrow,
                        ncol);
        Free2DArray((void **)unwrappedphase, nrow);
      }
    }
  ...
  if (!allmasked) {
    ...
    while (TRUE) {
      ...
      SetupIncrFlowCosts(costs, incrcosts, flows, nflow, nrow, narcrow,
                         narcsperrow, params);
      ...
      SetupTreeSolveNetwork(nodes, ground, apexes, iscandidate, nnoderow,
                            nnodesperrow, narcrow, narcsperrow, nrow, ncol);
      ...
      for (isource = 0; isource < nsource; isource++) {
        ...
        n += TreeSolve(nodes, NULL, ground, source, &candidatelist,
                       &candidatebag, &candidatelistsize, &candidatebagsize,
      ...
      if (nnondecreasedcostiter >= 2 * mostflow) {
        fflush(NULL);
        fprintf(sp0,
                "WARNING: No overall cost reduction for too many iterations."
                "  Breaking loop\n");
        break;
      }

      /* break if we're done with all flow increments or problem is convex */
      if (nflowdone >= params->maxflow || nflowdone >= mostflow ||
          params->p >= 1.0) {
        break;
      }
  ...
  if (params->ntilerow != 1 || params->ntilecol != 1) {
    GrowRegions(costs, flows, nrow, ncol, incrcosts, outfiles, tileparams,
                params);
  }
  ...
  fprintf(sp1, "Writing output to file %s\n", outfiles->outfile);
  WriteOutputFile(mag, unwrappedphase, outfiles->outfile, outfiles, nrow, ncol);
