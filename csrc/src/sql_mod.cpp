...
std::vector<std::string> get_param(sqlite3* db, const std::string& tblname, const std::string& name) {
    ...
    std::string sql = "SELECT name, value, units, type, comments FROM " + tblname + " WHERE name = ?";
    ...
    if (sqlite3_bind_text(stmt, 1, trimmed_name.c_str(), -1, SQLITE_STATIC) != SQLITE_OK) {
        sqlite3_finalize(stmt);
        throw std::runtime_error("SQL binding failed: " + std::string(sqlite3_errmsg(db)));
    }
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        result.push_back(reinterpret_cast<const char*>(sqlite3_column_text(stmt, 0))); // name
        result.push_back(reinterpret_cast<const char*>(sqlite3_column_text(stmt, 1))); // value
        result.push_back(reinterpret_cast<const char*>(sqlite3_column_text(stmt, 2))); // units
        result.push_back(reinterpret_cast<const char*>(sqlite3_column_text(stmt, 3))); // type
        result.push_back(reinterpret_cast<const char*>(sqlite3_column_text(stmt, 4))); // comments
    } else {
