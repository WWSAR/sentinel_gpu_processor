...
__host__ __device__ void llh2xyz(double llh[3], double xyz[3], const double r_a,
                                 const double r_e2) {
...
__host__ __device__ void xyz2llh(double *xyz, double *llh, const double r_a,
                                 const double r_e2) {
  ...
  r_tant = (z / r_p) * r_q;
  r_theta = atan(r_tant);
  r_tant = (z + r_q3 * r_b * pow(sin(r_theta), 3)) /
           (r_p - r_e2 * r_a * pow(cos(r_theta), 3));
  lat = atan(r_tant);
...
__host__ __device__ void xyz2llh(float *xyz, double *llh, const double r_a,
                                 const double r_e2) {
  double x, y, z, r_q2, r_q, r_q3, r_b, r_p, r_re, r_tant, r_theta, lon, lat, h;
  x = xyz[0];
  y = xyz[1];
...
void read_orbit(const std::string orbitfile, size_t &nstatvec, double **t,
                double **x, double **v) {
  double *orbitdata;
...
void read_orbit_ascii(const std::string orbitfile, size_t &nstatvec, double **t,
                      double **x, double **v) {
...
__host__ __device__ void orbithermite(double *tt, double *xx, double *vv,
...
__host__ __device__ void intp_orbit(const std::size_t nstatvec,
...
orbitrangetime(const std::size_t nstatvec, double *timeorbit, double *xx,
               double *vv, double *xyz, const double tline0, double *satx0,
               double *satv0, double &tline, double *dr) {
