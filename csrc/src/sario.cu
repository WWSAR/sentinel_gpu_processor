...
void readdem(const std::string &imgfile, const std::size_t toskip,
             const std::size_t n, short int *dem) {
...
void readdem(const std::string &imgfile, const std::size_t n, short int *dem) {
...
void write_compressed_strips(int32_t *header, const float2 *img, const int nrow,
  ...
  for (int i = 0; i < nrow; ++i) {
    ...
    if (!in_strip && row_has_data) {
      ...
      fs.write(reinterpret_cast<const char *>(&img[(start_row - top) * ncol]),
               strip_size);
