    ...
    int rowstart)
{
    ...
    // direct binary read
    fb.read(
        reinterpret_cast<char*>(stack + base_idx), nelement * sizeof(Complex));
 ...
 * @param ph_sim Phase similarity (can be either median or maximum phase similarity)
 * @param threshold Phase similarity threshold
 * @param overwrite Overwrite exisiting PS pixels
 * @param n Number of radar pixels
 */
__global__ void update_ps(
    int32_t* __restrict__ ps,
 ...
 */
template<int NNEIGH>
__global__ void median_similarity(
    const Complex* __restrict__ stack,
    const std::size_t nifg,
    const std::size_t nrow,
    const std::size_t ncol,
    const int rowstart_comp,
    const int rowend_comp,
    const int rowstart,
    const int rowend,
    ...
    if (r0 >= rowend_comp || c0 >= (int)ncol) return;
    if (!ps[r0 * ncol + c0]) return;
...
__global__ void maximum_similarity(
    const Complex* __restrict__ stack,
    const std::size_t nifg,
    const std::size_t nrow,
    const std::size_t ncol,
...
void similarity(
        ...
        const std::string& max_sim_outfile,
        const int nneigh,
        const int rdmin,
        const int rdmax,
        const float med_sim_th){
    // Declaration ============
    // Image stack
    ...
    std::cout << "interferograms are divided into " << nbatch <<
                 " batch(es)" << std::endl;
    std::cout << "Each batch consists of " << batch_lines << "rows" <<
        std::endl;
    total_size = (std::size_t)nifg *
                 std::min(nrow, batch_lines + 2*rdmax) *
                 ncol;
    ...
    for (int i = 0; i < nbatch; i++){
        ...
        std::cout << "Data loading, first row: " << rowstart << ", last row: "
                  << rowend << std::endl;
        ...
        CHECK_CUDA(cudaMemcpy(d_stack, stack, sizeof(Complex)*total_size,
                    cudaMemcpyHostToDevice));
        // data normlization
        normalize<<<num_blocks, block_size>>>(d_stack,
                (std::size_t)nifg*(rowend-rowstart)*ncol);
        CHECK_CUDA(cudaDeviceSynchronize());
        // compute median similarity
        if (nneigh < 32){
            median_similarity<32><<<grid,block>>>(
                    d_stack, nifg, nrow, ncol, rowstart_comp, rowend_comp,
                    rowstart, rowend, d_ps, nneigh, d_indices, indices.size,
                    d_med_sim);
        }else if(nneigh < 64){
...
int main(int argc, char *argv[]){
    ...
    similarity(infile, psfile, med_sim_outfile, max_sim_outfile, nneigh, rdmin, rdmax,
               med_sim_th);
    return 0;
}
