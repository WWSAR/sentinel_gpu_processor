...
class ScopedTimer {
    ...
    ~ScopedTimer() {
        ...
        std::cout << "[TIMER] " << name_ << " Elapsed Time: " << duration << " s" << std::endl;
    }
 ...
 */
std::vector<std::array<std::string, 3>> parse_input_file(
    ...
    while (std::getline(infile, line)) {
        ...
        if (iss >> row[0] >> row[1] >> row[2]) {
            ...
            std::cerr << "Warning: skipping a row with incorrect format" <<
                line << std::endl;
        }
    }
...
int crossmul(const std::string &input_file,
    ...
    for (const auto &slc_pair : slc_pairs){
        ...
        read_and_resample<Complex>(slc_pair[0], slc1, left, top, right, bottom,
                0, bottom1 - top1);
        read_and_resample<Complex>(slc_pair[1], slc2, left, top, right, bottom,
                0, bottom2 - top2);
        ...
        if (out_float){
            ...
            CHECK_CUDA(cudaMemcpy(
                phase,d_phase,sizeof(float)*nrow_sm*ncol_sm,
                cudaMemcpyDeviceToHost));
        }else{
            CHECK_CUDA(cudaMemcpy(
                ifglook,d_ifglook,sizeof(Complex)*nrow_sm*ncol_sm,
                cudaMemcpyDeviceToHost));
        }
        if (out_float){
            save_binary<float>(phase,nrow_sm*ncol_sm,ifg_header,
                NHEADER,slc_pair[2]);
        }else{
            save_binary<Complex>(ifglook,nrow_sm*ncol_sm,ifg_header,
                NHEADER,slc_pair[2]);
        }
    }
