...
void tcnbasis(double pos[3], double vel[3], double r_t[3],
              double r_c[3], double r_n[3], const std::string& look_dir = "RIGHT",
...
double ellipsoidal_hdg(double* ll1, double* ll2, double r_a = RA, double r_e2 = RE2) {
    ...
    double r_k2 = (16.0 * r_e2 * r_n * r_n) / (16.0 * r_e2 * r_n * r_n + r_ep * r_ep);
    double r_k3 = (16.0 * r_e2 * r_n * r_n) / (r_ep * r_ep);
    double r_k4 = (16.0 * r_n - r_ep * r_ep) / (16.0 * r_e2 * r_n * r_n + r_ep * r_ep);
    ...
    double r_sinphi = sign(r_sinlon) * sqrt(1.0 - std::min(r_cosphi * r_cosphi, 1.0));
    ...
    if (r_a * fabs(r_phi) > 1e-6) {
        ...
        double r_x = ((r_phi * (r_k4 + r_cb) + r_sinphi * (r_cc + r_cd) + r_cg * (r_cf + r_ce)) * r_ca) / r_k5;
        ...
        double r_ch = r_b0 * (1.0 + (r_q * r_ep * r_ep) / 4.0 - (3.0 * r_q * r_q * r_ep * r_ep * r_ep * r_ep) / 64.0);
        double r_ci = r_b0 * ((r_q * r_ep * r_ep) / 4.0 - ((r_q * r_q) * r_ep * r_ep * r_ep * r_ep) / 16.0);
...
void rah2ll(double *rah, double *tt, double *xx, double *vv,
    ...
    intp_orbit(nstatvec,tt,xx,vv,(start_time+end_time)/2.,xyzsatmid,velsatmid);
    ...
    for (int i = 0; i < 8; ++i){
        ...
        gamm = costheta*r;
        alpha = (dopfact*r-gamm*dot_product(nhat,vhat))/dot_product(vhat,that);
