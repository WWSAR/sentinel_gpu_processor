...
int deramp(const std::string &dbname, const std::string &burstfile,
  ...
  for (int iburst = 0; iburst < azimuth_bursts; iburst++) {
    std::cout << "processing burst " << (iburst + 1) << "/" << azimuth_bursts
              << std::endl;
    ...
    vs = sqrt(vpoint[0] * vpoint[0] + vpoint[1] * vpoint[1] +
              vpoint[2] * vpoint[2]);
    ks = 2.0 * vs * radar_frequency / 299792458.0 * azimuth_steering_rate *
         M_PI / 180.0;
    ...
    if (iburst == 0) {
      ...
      save_binary<Complex>(burst, true, nrange * lines_per_burst, burstoutfile);
